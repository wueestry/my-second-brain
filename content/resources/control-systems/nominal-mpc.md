---
{"publish":true,"title":"Nominal MPC","created":"2025-04-01 16:24","tags":["#control-systems","#resource","#term"],"cssclasses":""}
---
...
The core of nominal MPC involves solving an optimization problem at each time step `k`:
...
- $B$: Input matrix (describing how inputs affect the state).
- $Q$: State weighting matrix (positive semi-definite). Penalizes deviations from the desired state.
- $R$: Input weighting matrix (positive definite). Penalizes large control efforts.
...
- The `A` and `B` matrices are derived from the linearized system model.
